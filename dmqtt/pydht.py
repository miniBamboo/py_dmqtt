...
import random
import socket
# import  SocketServer #has be named socketserver
...
class DHTRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        try:
            ...
            elif message_type == "delete": # Add by Fu Ming Chen at 2021.3.3
    ...
    # Add by Fu Ming Chen at 2021.3.3
    def handle_delete(self, message):
        key = message["id"]
        self.server.dht.data.pop(key)
...
class DHT(object):
    ...
    def delete(self, key):
        ...
        nearest_nodes = self.iterative_find_nodes(hashed_key)
        if hashed_key in self.data:
            value = self.data.pop(hashed_key)
            for node in nearest_nodes:
    ...
    def get_nodes(self,key):
        hashed_key = hash_function(key)
        nodes = self.buckets.nearest_nodes(hashed_key, limit=alpha)
        nodes_adress = [peer.address() for peer in nodes]
        return nodes_adress
